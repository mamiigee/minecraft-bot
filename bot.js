...
class AFKBotManager {
    ...
    // --- YENİ EKLENEN ALTIN ÇİFTLİĞİ VE DÖNGÜ METOTLARI ---

    async runGoldFarmLoop(id, loopCount) {
        const botInstance = this.bots.get(id);
        if (!botInstance || !botInstance.bot) {
            return { status: "error", message: "Bot aktif değil!" };
        }
        const bot = botInstance.bot;
        let count = parseInt(loopCount) || 1;

        try {
            console.log(`[BOT:${id}] Altın çiftliği döngüsü başlatıldı. Toplam döngü sayısı: ${count}`);
            
            for (let i = 1; i <= count; i++) {
                console.log(`[BOT:${id}] --- Döngü ${i} / ${count} başladı ---`);
                
                let filled = await this.fillInventoryWithGold(bot);
                if (!filled) {
                    console.log(`[BOT:${id}] Envanter doldurulamadı veya /fm menüsünde altın bitti. Döngü durduruluyor.`);
                    return { status: "error", message: `Döngü ${i} aşamasında altın toplanamadı.` };
                }
                
                await new Promise(resolve => setTimeout(resolve, 500));
                
                let craftedAndDropped = await this.craftAndDropGoldBlocks(bot);
                if (!craftedAndDropped) {
                    console.log(`[BOT:${id}] Blok yapma veya atma başarısız oldu. Döngü durduruluyor.`);
                    return { status: "error", message: `Döngü ${i} aşamasında blok üretilemedi.` };
                }

                console.log(`[BOT:${id}] --- Döngü ${i} başarıyla tamamlandı ---`);
                await new Promise(resolve => setTimeout(resolve, 1500));
            }

            console.log(`[BOT:${id}] Belirlenen tüm döngüler başarıyla bitti!`);
            return { status: "success", message: `${count} döngü başarıyla tamamlandı!` };

        } catch (err) {
            console.log(`[BOT:${id}] Döngü genel hatası: ${err.message}`);
            return { status: "error", message: err.message };
        }
    }

    async fillInventoryWithGold(bot) {
        try {
            while (bot.inventory.emptySlotCount() > 2) {
                bot.chat('/fm');
                const window = await this.openWindow(bot);
                if (!window) return false;

                let goldSlot = -1;
                for (let i = 0; i < window.slots.length; i++) {
                    const item = window.slots[i];
                    if (item && (item.name.includes('gold') || item.displayName.toLowerCase().includes('altın'))) {
                        goldSlot = i;
                        break;
                    }
                }
                ...
                await bot.clickWindow(goldSlot, 1, 0);
    ...
    async craftAndDropGoldBlocks(bot) {
        try {
            ...
            let totalIngots = bot.inventory.items()
                .filter(item => item.name === 'gold_ingot')
                .reduce((sum, item) => sum + item.count, 0);

            if (totalIngots < 9) return false;

            let maxBlocksToCraft = Math.floor(totalIngots / 9);
            const goldBlockType = bot.registry.itemsByName['gold_block'];
            const recipes = bot.recipesFor(goldBlockType.id, null, maxBlocksToCraft, craftingTable);
            if (recipes.length === 0) return false;

            await bot.craft(recipes[0], maxBlocksToCraft, craftingTable);
            await new Promise(resolve => setTimeout(resolve, 500));

            const goldBlocks = bot.inventory.items().filter(item => item.name === 'gold_block');
            for (const blockItem of goldBlocks) {
                await bot.tossStack(blockItem);
